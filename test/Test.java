...
public class Test extends JFrame {
    ...
    private void loadSemesters() {
        try (Connection conn = DatabaseConnection.getInstance().getConnection(); Statement stmt = conn.createStatement(); ResultSet rs = stmt.executeQuery("SELECT MaHocKy, TenHocKy FROM HocKy")) {
    ...
    private void updateSchedule() {
        ...
        for (int i = 0; i < 7; i++) {
            ...
            rowData[0] = dayName;
            try (Connection conn = DatabaseConnection.getInstance().getConnection(); PreparedStatement stmt = conn.prepareStatement(
